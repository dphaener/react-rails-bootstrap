...
This is an ejected version of a `create-react-app` application. It has been
slightly modified to work with Relay.
...
The convention here is to put all components in the `src/components` directory
and colocate all CSS, Mutations, and business logic that is coupled to that
component:
   ...
   +- components
      +- user
         ...
         +- mutations
            ...
            +- UpdateUserMutation.js
            +- DestroyUserMutation.js
         +- mutators.js
```

All mutations should be exported from the `index.js` file for easy import.

We are using a concept of a `mutators` file that contains all the logic
for applying Relay mutations to a resource.

If Redux is implemented, reducers and actions should also be colocated in the
folder for the component.

## Utilities

The set of utilities is currently small, but growing. Currently everything
lives in the `lib` directory, and is mostly self explanatory.
