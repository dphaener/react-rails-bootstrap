...
#{model.to_s}Type = GraphQL::ObjectType.define do
  ...
  field :#{ast.to_s} do
    ...
    resolve #{model.class.name}Resolver
  end
              FILE
              end
            end
          ...
          begin
            if models.include? association_klass.classify.constantize
              if circular_finder[model.to_s].include? association_klass || association_klass == model.to_s
                inject_into_file type_path(model), before: "# End of fields\n" do <<-FILE
  connection :#{ast.to_s} do
    type -> { #{model.reflect_on_association(ast).class_name}Type.connection_type }
    resolve #{model.class.name}Resolver.new.collection
  end
                FILE
                end
              else
...
module #{model.to_s}Mutations
  Create = GraphQL::Relay::Mutation.define do
    ...
    resolve #{model.to_s}Resolver::Create
  end
  ...
  Update = GraphQL::Relay::Mutation.define do
    ...
    resolve #{model.to_s}Resolver::Update
  end
  ...
  Destroy = GraphQL::Relay::Mutation.define do
    ...
    resolve #{model.to_s}Resolver::Destroy
  end
end
        FILE
      end
    ...
    input_field :#{columns[k].name}, !types.#{type_mapper[columns[k].type]}
      ...
      def add_mutation_query(model)
        inject_into_file "app/graphql/mutation_type.rb", after: "name 'MutationType'\n" do <<-FILE
  field :create#{model.to_s}, #{model.to_s}Mutations::Create.field
  field :update#{model.to_s}, #{model.to_s}Mutations::Update.field
  field :destroy#{model.to_s}, #{model.to_s}Mutations::Destroy.field
        FILE
        end
      end
